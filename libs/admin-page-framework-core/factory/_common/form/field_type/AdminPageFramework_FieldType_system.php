...
class AdminPageFramework_FieldType_system extends AdminPageFramework_FieldType {
    ...
    protected function getStyles() {
        return ".DLDC-field-system {width: 100%;}.DLDC-field-system .DLDC-input-label-container {width: 100%;}.DLDC-field-system textarea {background-color: #f9f9f9; width: 97%; outline: 0; font-family: Consolas, Monaco, monospace;white-space: pre;word-wrap: normal;overflow-x: scroll;}";
    }
    protected function getField($aField) {
        $_aInputAttributes = $aField['attributes'];
        $_aInputAttributes['class'].= ' system';
        unset($_aInputAttributes['value']);
        return $aField['before_label'] . "<div class='DLDC-input-label-container'>" . "<label for='{$aField['input_id']}'>" . $aField['before_input'] . ($aField['label'] && !$aField['repeatable'] ? "<span " . $this->getLabelContainerAttributes($aField, 'DLDC-input-label-string') . ">" . $aField['label'] . "</span>" : "") . "<textarea " . $this->getAttributes($_aInputAttributes) . " >" . esc_textarea($this->___getSystemInformation($aField['value'], $aField['data'], $aField['print_type'])) . "</textarea>" . $aField['after_input'] . "</label>" . "</div>" . $aField['after_label'];
    }
    ...
    private function ___getClientInfo($bGenerateInfo = true) {
        ...
        return empty($_aBrowser) ? __('No browser information found.', 'DLDC') : $_aBrowser;
    }
    ...
    private function ___getSiteInfo() {
        global $wpdb;
        return array(__('Version', 'DLDC') => $GLOBALS['wp_version'], __('Language', 'DLDC') => $this->getSiteLanguage(), __('Memory Limit', 'DLDC') => $this->getReadableBytes($this->getNumberOfReadableSize(WP_MEMORY_LIMIT)), __('Multi-site', 'DLDC') => $this->getAOrB(is_multisite(), $this->oMsg->get('yes'), $this->oMsg->get('no')), __('Permalink Structure', 'DLDC') => get_option('permalink_structure'), __('Active Theme', 'DLDC') => $this->___getActiveThemeName(), __('Registered Post Statuses', 'DLDC') => implode(', ', get_post_stati()), 'WP_DEBUG' => $this->getAOrB($this->isDebugMode(), $this->oMsg->get('enabled'), $this->oMsg->get('disabled')), 'WP_DEBUG_LOG' => $this->getAOrB($this->isDebugLogEnabled(), $this->oMsg->get('enabled'), $this->oMsg->get('disabled')), 'WP_DEBUG_DISPLAY' => $this->getAOrB($this->isDebugDisplayEnabled(), $this->oMsg->get('enabled'), $this->oMsg->get('disabled')), __('Table Prefix', 'DLDC') => $wpdb->prefix, __('Table Prefix Length', 'DLDC') => strlen($wpdb->prefix), __('Table Prefix Status', 'DLDC') => $this->getAOrB(strlen($wpdb->prefix) > 16, $this->oMsg->get('too_long'), $this->oMsg->get('acceptable')), 'wp_remote_post()' => $this->___getWPRemotePostStatus(), 'wp_remote_get()' => $this->___getWPRemoteGetStatus(), __('Multibite String Extension', 'DLDC') => $this->getAOrB(function_exists('mb_detect_encoding'), $this->oMsg->get('enabled'), $this->oMsg->get('disabled')), __('WP_CONTENT_DIR Writeable', 'DLDC') => $this->getAOrB(is_writable(WP_CONTENT_DIR), $this->oMsg->get('yes'), $this->oMsg->get('no')), __('Active Plugins', 'DLDC') => PHP_EOL . $this->___getActivePlugins(), __('Network Active Plugins', 'DLDC') => PHP_EOL . $this->___getNetworkActivePlugins(), __('Constants', 'DLDC') => $this->___getDefinedConstants('user'),);
    }
    ...
    private function _getPHPInfo($bGenerateInfo = true) {
        ...
        self::$_aPHPInfo = array(__('Version', 'DLDC') => phpversion(), __('Safe Mode', 'DLDC') => $this->getAOrB(@ini_get('safe_mode'), $this->oMsg->get('yes'), $this->oMsg->get('no')), __('Memory Limit', 'DLDC') => @ini_get('memory_limit'), __('Upload Max Size', 'DLDC') => @ini_get('upload_max_filesize'), __('Post Max Size', 'DLDC') => @ini_get('post_max_size'), __('Upload Max File Size', 'DLDC') => @ini_get('upload_max_filesize'), __('Max Execution Time', 'DLDC') => @ini_get('max_execution_time'), __('Max Input Vars', 'DLDC') => @ini_get('max_input_vars'), __('Argument Separator', 'DLDC') => @ini_get('arg_separator.output'), __('Allow URL File Open', 'DLDC') => $this->getAOrB(@ini_get('allow_url_fopen'), $this->oMsg->get('yes'), $this->oMsg->get('no')), __('Display Errors', 'DLDC') => $this->getAOrB(@ini_get('display_errors'), $this->oMsg->get('on'), $this->oMsg->get('off')), __('Log Errors', 'DLDC') => $this->getAOrB(@ini_get('log_errors'), $this->oMsg->get('on'), $this->oMsg->get('off')), __('Error log location', 'DLDC') => @ini_get('error_log'), __('Error Reporting Level', 'DLDC') => $_oErrorReporting->getErrorLevel(), __('FSOCKOPEN', 'DLDC') => $this->getAOrB(function_exists('fsockopen'), $this->oMsg->get('supported'), $this->oMsg->get('not_supported')), __('cURL', 'DLDC') => $this->getAOrB(function_exists('curl_init'), $this->oMsg->get('supported'), $this->oMsg->get('not_supported')), __('SOAP', 'DLDC') => $this->getAOrB(class_exists('SoapClient'), $this->oMsg->get('supported'), $this->oMsg->get('not_supported')), __('SUHOSIN', 'DLDC') => $this->getAOrB(extension_loaded('suhosin'), $this->oMsg->get('supported'), $this->oMsg->get('not_supported')), 'ini_set()' => $this->getAOrB(function_exists('ini_set'), $this->oMsg->get('supported'), $this->oMsg->get('not_supported')),) + $this->getPHPInfo() + array(__('Constants', 'DLDC') => $this->___getDefinedConstants(null, 'user'));
        return self::$_aPHPInfo;
    }
    private function ___getWebServerInfo($bGenerateInfo = true) {
        return $bGenerateInfo ? array(__('Web Server', 'DLDC') => $_SERVER['SERVER_SOFTWARE'], 'SSL' => $this->getAOrB(is_ssl(), $this->oMsg->get('yes'), $this->oMsg->get('no')), __('Session', 'DLDC') => $this->getAOrB(isset($_SESSION), $this->oMsg->get('enabled'), $this->oMsg->get('disabled')), __('Session Name', 'DLDC') => esc_html(@ini_get('session.name')), __('Session Cookie Path', 'DLDC') => esc_html(@ini_get('session.cookie_path')), __('Session Save Path', 'DLDC') => esc_html(@ini_get('session.save_path')), __('Session Use Cookies', 'DLDC') => $this->getAOrB(@ini_get('session.use_cookies'), $this->oMsg->get('on'), $this->oMsg->get('off')), __('Session Use Only Cookies', 'DLDC') => $this->getAOrB(@ini_get('session.use_only_cookies'), $this->oMsg->get('on'), $this->oMsg->get('off')),) + $_SERVER : '';
    }
    }
