...
( function( $ ) {
    ...
    $.fn.updateAdminPageFrameworkRepeatableSections = function( aSettings ) {
        ...
        var _oThis                = this;
        var _sSectionsContainerID = _oThis.find( '.repeatable-section-add-button' ).first().closest( '.DLDC-sections' ).attr( 'id' );
        ...
        var _sSectionID           = _oThis.find( '.repeatable-section-add-button' ).first().closest( '.DLDC-section' ).attr( 'id' );
        var _nCurrentSectionCount = jQuery( '#' + _sSectionsContainerID ).find( '.DLDC-section' ).length;
    ...
    $.fn.addAdminPageFrameworkRepeatableSection = function( sSectionContainerID ) {
        ...
        if ( 'undefined' === typeof sSectionContainerID ) {
            var sSectionContainerID = $( this ).closest( '.DLDC-section' ).attr( 'id' );
        }
        var nodeSectionContainer    = $( '#' + sSectionContainerID );
        var nodeNewSection          = nodeSectionContainer.clone(); // clone without bind events.
        var nodeSectionsContainer   = nodeSectionContainer.closest( '.DLDC-sections' );
        var sSectionsContainerID    = nodeSectionsContainer.attr( 'id' );        
        var nodeTabsContainer       = $( this ).closest( '.DLDC-section-tabs-contents' )
            .children( '.DLDC-section-tabs' )
            .first();
        ...
        if ( _sMaxNumberOfSections != 0 && nodeSectionsContainer.find( '.DLDC-section' ).length >= _sMaxNumberOfSections ) {
            var _nodeLastRepeaterButtons = nodeSectionContainer.find( '.DLDC-repeatable-section-buttons' ).last();
        ...
        var _sSectionTabSlug = nodeNewSection.find( '.DLDC-section-caption' ).first().attr( 'data-section_tab' );
        if ( ! _sSectionTabSlug || _sSectionTabSlug === '_default' ) {
            nodeNewSection.find( '.DLDC-section-title' ).not( '.DLDC-collapsible-section-title' ).hide();
        }
        ...
        if( 'function' === typeof nodeNewSection.enableAdminPageFrameworkCollapsibleButton ){ 
            nodeNewSection.find( '.DLDC-collapsible-sections-title, .DLDC-collapsible-section-title' ).enableAdminPageFrameworkCollapsibleButton();
        }
        ...
        $( nodeNewSection ).find( '.DLDC-field' ).each( function( iFieldIndex ) {
            ...
            $( this ).trigger( 
                ...
                [
                    ...
                    jQuery( nodeNewSection ).closest( '.DLDC-sections' )    // model container
                ]
            );              
        ...
        nodeNewSection.find( '.DLDC-fields.sortable' ).each( function() {
        ...
        if ( nodeTabsContainer.length > 0 && ! nodeSectionContainer.hasClass( 'is_subsection_collapsible' ) ) {
            ...
            nodeTabsContainer.closest( '.DLDC-section-tabs-contents' ).createTabs( 'refresh' );
        ...
        var _incrementAttributes = function( oElement, iSectionsCount, oSectionsContainer ) {
            ...
            $( oElement ).find( 'tr.DLDC-fieldrow, .DLDC-fieldset, .DLDC-fields, .DLDC-field, table.form-table, input,textarea,select,option' )
                .incrementAttribute( 
                    'id', 
                    iSectionsCount,
                    _sSectionIDModel
            ...
            $( oElement ).find( '.DLDC-fields' ).incrementAttribute( 
                'data-field_tag_id_model',
                iSectionsCount,
                _sSectionIDModel
            );
            $( oElement ).find( '.DLDC-fields' ).incrementAttributes( 
        ...
        // @todo this may be able to be removed
            $( oElement ).find( '.DLDC-fieldset' ).incrementAttribute( 
                'data-field_id',
                iSectionsCount,
                _sSectionIDModel
            );
    ...
    $.fn.removeAdminPageFrameworkRepeatableSection = function() {
        ...
        var nodeSectionContainer    = $( this ).closest( '.DLDC-section' );
        var sSectionContainerID     = nodeSectionContainer.attr( 'id' );
        var nodeSectionsContainer   = $( this ).closest( '.DLDC-sections' );
        var sSectionsContainerID    = nodeSectionsContainer.attr( 'id' );        
        var nodeTabsContainer       = $( this ).closest( '.DLDC-section-tabs-contents' )
            .children( '.DLDC-section-tabs' )
            .first();
        var nodeTabs                = nodeTabsContainer.children( '.DLDC-section-tab' );
        ...
        if ( _sMinNumberOfSections != 0 && nodeSectionsContainer.find( '.DLDC-section' ).length <= _sMinNumberOfSections ) {
            var _nodeLastRepeaterButtons = nodeSectionContainer.find( '.DLDC-repeatable-section-buttons' ).last();
        ...
        nodeSectionContainer.fadeOut( _iFadeout, function() { 
            ...
            if ( 1 === _nodeRemoveButtons.length ) {
                ...
                var _sSectionTabSlug = nodeSectionsContainer.find( '.DLDC-section-caption' ).first().attr( 'data-section_tab' );
                if ( ! _sSectionTabSlug || '_default' === _sSectionTabSlug ) {
                    nodeSectionsContainer.find( '.DLDC-section-title' ).first().show();
                }                
            }            
...
var _iSectionIndex = _iIterationIndex;
            ...
            // Call the registered callback functions.
            // @deprecated  3.6.0
            // $( this ).find( '.DLDC-field' ).each( function( iFieldIndex ) {    
...
jQuery( document ).ready( function() {
    ...
    // Adds the buttons
    jQuery( '#{$sContainerTagID} .DLDC-section-caption' ).each( function(){
        
        jQuery( this ).show();
        
        var _oButtons = jQuery( $_sButtonsHTML );
        if ( jQuery( this ).children( '.DLDC-collapsible-section-title' ).children( 'fieldset' ).length > 0 ) {
            _oButtons.addClass( 'section_title_field_sibling' );
        }
        var _oCollapsibleSectionTitle = jQuery( this ).find( '.DLDC-collapsible-section-title' );
...
JAVASCRIPTS;
        return "<script type='text/javascript' class='DLDC-section-repeatable-script'>" . '/* <![CDATA[ */' . $_sScript . '/* ]]> */' . "</script>";
    }
