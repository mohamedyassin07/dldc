...
(function ( $ ) {
    ...
    $.fn.updateAdminPageFrameworkRepeatableFields = function( aSettings ) {
        ...
        var _oRepeatableButtons = $( nodeThis ).find( '.DLDC-repeatable-field-buttons' )
            .filter( function() {                                       
                return $( this ).closest( '.DLDC-fields' ).attr( 'id' ) === _sFieldsContainerID;  // Avoid dealing with nested field's elements.                
            });       
        ...
        var _sFieldID           = _oRepeatableAddButtons.first().closest( '.DLDC-field' ).attr( 'id' );
        var _nCurrentFieldCount = $( '#' + _sFieldsContainerID ).find( '.DLDC-field' )
            .filter( function() {                 
                return $( this ).closest( '.DLDC-fields' ).attr( 'id' ) === _sFieldsContainerID;  // Avoid dealing with nested field's elements.                
            })        
            .length;
    ...
    $.fn.addAdminPageFrameworkRepeatableField = function( sFieldContainerID ) {
        ...
        if ( 'undefined' === typeof sFieldContainerID ) {
            var sFieldContainerID = $( this ).closest( '.DLDC-field' ).attr( 'id' );
        }

        var nodeFieldContainer  = $( '#' + sFieldContainerID );
        var nodeNewField        = nodeFieldContainer.clone(); // clone without bind events.
        var nodeFieldsContainer = nodeFieldContainer.closest( '.DLDC-fields' );
        ...
        if ( ! _aOptions ) {     
            var _nodeButtonContainer = nodeFieldContainer.find( '.DLDC-repeatable-field-buttons' )
                .filter( function() {                 
                    return $( this ).closest( '.DLDC-fields' ).attr( 'id' ) === _sFieldsContainerID;  // Avoid dealing with nested field's elements.                
                })
                .first();
        ...
        var _oInnerFields       = nodeFieldsContainer.find( '.DLDC-field' ) 
                                    .filter( function() {                 
                                        return $( this ).closest( '.DLDC-fields' ).attr( 'id' ) === _sFieldsContainerID;  // Avoid dealing with nested field's elements.                
                                    });
        var _oRepeatableButtons = nodeFieldContainer.find( '.DLDC-repeatable-field-buttons' )
                                    .filter( function() {                 
                                        return $( this ).closest( '.DLDC-fields' ).attr( 'id' ) === _sFieldsContainerID;  // Avoid dealing with nested field's elements.                
                                    }); 
        ...
        $( nodeNewField ).find( '.DLDC-field' ).addBack().trigger( 
            'DLDC_repeated_field', 
            [ 
                0, // call type, 0 : repeatable fields, 1: repeatable sections
                jQuery( nodeNewField ).closest( '.DLDC-fields' )    // model container
            ]
        );
        ...
        var nodeRemoveButtons = nodeFieldsContainer
            ...
            .filter( function() {                      
                return $( this ).closest( '.DLDC-fields' ).attr( 'id' ) === _sFieldsContainerID;                
            })
        ...
        nodeFieldsContainer.children( '.DLDC-field' ).children( '.delimiter' ).show().last().hide();
        ...
        var _incrementFieldAttributes = function( oElement, oFieldsContainer ) {
            ...
            oElement.find( '.DLDC-field,.DLDC-fields,.DLDC-fieldset' ).incrementAttributes(
    ...
    $.fn.removeAdminPageFrameworkRepeatableField = function() {
        ...
        var nodeFieldContainer  = $( this ).closest( '.DLDC-field' );
        var nodeFieldsContainer = $( this ).closest( '.DLDC-fields' );
        var _sFieldsContainerID = nodeFieldsContainer.attr( 'id' );
        var _aOptions = nodeFieldsContainer.data( 'repeatable' );
