(function($){
    ...
    function initializeSelect2( oNode ) {
        ...
        if ( _aOptions[ 'width' ] && 'auto' !== _aOptions[ 'width' ] ) {
            var _oFieldContainer = _oSelect2Target.closest( '.DLDC-field-select2' );
            _oFieldContainer.css( 'width', _aOptions[ 'width' ] );
            _oFieldContainer.children( '.DLDC-select-label' )
                .css( 'width', '100%' );
            _oFieldContainer.children( '.DLDC-select-label' )
                .children( 'label' )
                .children( '.DLDC-input-container' )
                .css( 'width', '100%' );
            _aOptions[ 'width' ] = '100%';
        }
        ...
        if ( _aOptions[ 'search_callback' ] ) {
             ...
             * Set initial values.
             */
            var _oInputForEncoded = _oSelect2Target.closest( '.DLDC-field' )
    ...
    function templateResultForIcon( state ) {
        var _oIconDefinition = $( state.element ).closest( '.DLDC-field' ).find( '.custom-icon-definition[data-value=' + state.id + ']' );
